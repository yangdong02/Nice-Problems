...
inline bool cmp2(const Query &q1, const Query &q2) {return q1.r < q2.r; }

ll gcd(ll x, ll y) { return y == 0LL ? x : gcd(y, x % y); }

inline void Ins(int v) { cur_ans += (2 * cnt[v] + 1), cnt[v]++; }
inline void Del(int v) { cur_ans -= (2 * cnt[v] - 1), cnt[v]--; }
...
int main() {
    ...
    for(int i = 1; i <= t; ++i) sort(q + L[t], q + R[t] + 1, cmp2);
